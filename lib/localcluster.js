...
var q = require('q')
var submit = require('./submitter').submit
var ttypes = require('./gen-nodejs/storm_types')
var wrench = require('wrench')

module.exports = LocalCluster

function waitForPortListener(port, timeout, callback) {
	var delay = 1000
...
LocalCluster.prototype._startServer = function(serverName, port) {
	...
	if (port != null) {
		return q.nfcall(waitForPortListener, port, 60000).then(function() {
			return server
		})
	} else {
...
LocalCluster.prototype._start = function() {
	...
	var afterStartup = self._startServer('dev-zookeeper', 2181).then(function(zookeeper) {
		return [zookeeper, self._startServer('nimbus', 6627), self._startServer('supervisor')]
	}).spread(function(zookeeper, nimbus, supervisor) {
		self._servers = [zookeeper, nimbus, supervisor]
...
LocalCluster.prototype._killTopologies = function() {
	...
	return q.all(this._topologies.map(function(topologyName) {
		return client.killTopologyWithOpts(topologyName, new ttypes.KillOptions({wait_secs:0}))
	})).then(function() {
...
LocalCluster.prototype.submit = function(topology, options, callback) {
	var self = this
	self._afterStartup.then(function() {
